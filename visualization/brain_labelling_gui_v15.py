...
class Mode(Enum):
    ...
    # REVIEW_GLOBAL_PROPOSAL = 'review global proposal'
    # REVIEW_LOCAL_PROPOSAL = 'review local proposal'
...
class BrainLabelingGUI(QMainWindow, Ui_BrainLabelingGui):
	...
	def on_pick(self, event):
		...
		if event.artist in self.freeform_polygons:
			...
			if len(polygon_ids) > 0: # picked on a vertex circle
				...
				self.curr_proposal_id = self.curr_freeform_polygon_id
			...
			if hasattr(self, 'global_proposal_pathPatches') and event.artist in self.global_proposal_pathPatches:
				self.curr_proposal_id = self.global_proposal_pathPatches.index(event.artist)
				self.curr_proposal_type = ProposalType.GLOBAL
				self.statusBar().showMessage('picked global proposal %d (%s)' % (self.curr_proposal_id, self.global_proposal_labels[self.curr_proposal_id]))

			elif hasattr(self, 'local_proposal_pathPatches') and event.artist in self.local_proposal_pathPatches:
				self.curr_proposal_id = self.local_proposal_pathPatches.index(event.artist)
				self.curr_proposal_type = ProposalType.LOCAL
				self.statusBar().showMessage('picked local proposal %d (%s)' % (self.curr_proposal_id, self.local_proposal_labels[self.curr_proposal_id]))
	...
	def load_callback(self):
		...
		if suffix == 'globalProposalReviewResult':
			self.load_global_proposals()
			self.global_proposal_labels, self.global_proposal_review_results = self.dm.load_proposal_review_result(username, timestamp, suffix)
			
			for i, r in enumerate(self.global_proposal_review_results):
				if r == 1:
					self.global_proposal_pathPatches[i] = self.pathPatch_from_dedges(self.global_proposal_dedges[i], color=self.boundary_colors[1])
					self.global_proposal_pathPatches[i].set_picker(True)
					self.axis.add_patch(self.global_proposal_pathPatches[i])
		...
		elif suffix == 'localProposalReviewResult':
			self.load_local_proposals()
			self.local_proposal_labels, self.local_proposal_review_results = self.dm.load_proposal_review_result(username, timestamp, suffix)

			for i, r in enumerate(self.local_proposal_review_results):
				if r == 1:
					self.local_proposal_pathPatches[i] = self.pathPatch_from_dedges(self.local_proposal_dedges[i], color=self.boundary_colors[1])
					self.local_proposal_pathPatches[i].set_picker(True)
					self.axis.add_patch(self.local_proposal_pathPatches[i])

		elif suffix == 'freeformProposals':

			init_freeform_polygons_dict = self.dm.load_proposal_review_result(username, timestamp, suffix)

			for label, typed_polygons in init_freeform_polygons_dict.iteritems():
				for polygon_type, vertices in typed_polygons:
					...
					self.freeform_polygons.append(polygon)
					# self.polygon_bbox_list.append(x0_y0_x1_y1)
					self.freeform_proposal_labels.append(label)
					self.polygon_types.append(polygon_type)
	...
	def set_selected_proposal_label(self, label):
		...
		if self.curr_proposal_type == ProposalType.GLOBAL:
			self.global_proposal_labels[self.curr_proposal_id] = label
		elif self.curr_proposal_type == ProposalType.LOCAL:
			self.local_proposal_labels[self.curr_proposal_id] = label
		elif self.curr_proposal_type == ProposalType.FREEFORM:
			self.freeform_proposal_labels[self.curr_proposal_id] = label
	...
	def accProp_callback(self):
		...
		if self.curr_proposal_type == ProposalType.GLOBAL:
			self.global_proposal_review_results[self.curr_proposal_id] = 1

		elif self.curr_proposal_type == ProposalType.LOCAL:
			self.local_proposal_review_results[self.curr_proposal_id] = 1
	...
	def rejProp_callback(self):
		...
		if self.curr_proposal_type == ProposalType.GLOBAL:
			self.global_proposal_review_results[self.curr_proposal_id] = 2
		elif self.curr_proposal_type == ProposalType.LOCAL:
			self.local_proposal_review_results[self.curr_proposal_id] = 2

		self.curr_proposal_pathPatch.set_color(self.boundary_colors[2])
		self.curr_proposal_pathPatch.set_picker(False)
		self.canvas.draw()
	...
	def show_global_proposal_covering_sp(self, sp_ind):
		...
		decision = self.global_proposal_review_results[self.curr_proposal_id]
		...
		if self.global_proposal_labels[self.curr_proposal_id] is None:
			label = ''
		else:
			label = self.global_proposal_labels[self.curr_proposal_id]
	...
	def show_local_proposal_from_sp(self, sp_ind):
		...
		decision = self.local_proposal_review_results[self.curr_proposal_id]
		...
		if self.local_proposal_labels[self.curr_proposal_id] is None:
			label = ''
		else:
			label = self.local_proposal_labels[self.curr_proposal_id]
	...
	# def save_proposals(self):
		...
		if hasattr(self, 'global_proposal_review_results'):
			self.dm.save_proposal_review_result((self.global_proposal_labels, self.global_proposal_review_results), self.username, timestamp, suffix='globalProposalReviewResult')

		if hasattr(self, 'local_proposal_review_results'):
			self.dm.save_proposal_review_result((self.local_proposal_labels, self.local_proposal_review_results), self.username, timestamp, suffix='localProposalReviewResult')
		...
		if hasattr(self, 'freeform_polygons') and len(self.freeform_polygons) > 0:
			...
			self.dm.set_labelnames(sorted(list(set(self.dm.labelnames + self.new_labelnames))))
	...
	def cancel_current_proposal(self):
		if self.curr_proposal_pathPatch is not None:
			...
			if self.curr_proposal_pathPatch in self.axis.patches:
				if (self.curr_proposal_type == ProposalType.GLOBAL and self.global_proposal_review_results[self.curr_proposal_id] != 1) or \
					(self.curr_proposal_type == ProposalType.LOCAL and self.local_proposal_review_results[self.curr_proposal_id] != 1):
