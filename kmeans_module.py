...
class SaliencyDetector(object):
    ...
    def __init__(self, img, features_input):
        self.img = img
    ...
    def compute_texton(self, num_textons = 10):
        ...
        f = self.features.reshape(-1, self.features.shape[-1])
        kmeans_model.fit(f)
        self.textonmap = kmeans_model.predict(f).reshape(self.features.shape[:2])
    
    def segment_superpixels(self, compactness=5):
        self.segmentation = slic(gray2rgb(self.img), compactness=compactness, sigma=20)
    ...
    def compute_saliency_map(self):
        overall_texton_hist = np.bincount(self.textonmap.flat)
        ...
        for i, sp_hist in enumerate(self.sp_texton_hist_normalized):
            ...
            self.saliency_score[i] += 2.*neighbor_term/(len(self.neighbors[i])-1)

        self.saliency_map = self.saliency_score[self.segmentation]
    
    def visualize_saliency_map(self, output=None):
        plt.matshow(self.saliency_map, cmap=plt.cm.Greys_r)
        plt.colorbar()
        plt.show()
        
    def find_salient_clusters(self, dist_thresh = 0.5):
