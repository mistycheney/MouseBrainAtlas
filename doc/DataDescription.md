...
- a 3-D array stored as `bp` file. Naming convention is `<volume_spec>.bp`.
- a (3,) int array representing the origin of this array with respect to _wholebrain_ (see [Definition of frames]), stored as `txt` file. Naming convention is `<volume_spec>_origin_with_wholebrain.txt`.
