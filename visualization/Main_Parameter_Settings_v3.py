...
import os
import json
import param_settings_v3
import string



class MainDialog(QMainWindow,param_settings_v3.Ui_form):
	...
	self.statusBar().showMessage("selected gabor:  None Selected      selected segm:  None Selected      selected vq:  None Selected")
    ### Update list widgets ###
	...
	if self.list_gabor.on_item_clicked()==1:
	    ...
	    #Update status bar
	    try:
	    	a = str(self.list_gabor.currentItem().text())
	    except:
	    	a = "None Selected"	
	    try:	
	    	b = str(self.list_segm.currentItem().text())
	    except:
	    	b = "None Selected"
	    try:
	    	c = str(self.list_vq.currentItem().text())
	    except:
	    	c = "None Selected"	
	    self.statusBar().showMessage("selected gabor:  %s      selected segm:  %s      selected vq:  %s" % (a,b,c))
