...
for i in range(1, len(filenames)):
	...
	if os.path.exists(custom_tf_fn):
		...
		sys.stderr.write('%s' % [t11, t12, t13, t21, t22, t23])
...
# final_tranf_filename = os.path.join(elastix_output_dir, '%(stack)s_transformsTo_%(anchor_fn)s.pkl' % \
# 														dict(stack=stack, anchor_fn=filenames[anchor_idx]))

with open(output_fn, 'w') as f:
	pickle.dump(transformation_to_anchor_sec, f) # Note that the index starts at 0, BUT the .._renamed folder index starts at 1.

# transformation_to_anchor_fn = {}
# for moving_idx, T_to_anchor in transformation_to_anchor_sec.iteritems():
# 	transformation_to_anchor_fn[filenames[moving_idx]] = T_to_anchor
#
# with open(final_tranf_filename, 'w') as f:
# 	pickle.dump(transformation_to_anchor_fn, f)
