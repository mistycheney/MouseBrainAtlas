...
if os.path.exists(textonmap_file):
    ...
    textonmap_img_fn = os.path.join(args.cache_dir,
                                '%s_param%d_textonmap.png'%(img_name, params['param_id']))
    cv2.imwrite(textonmap_img_fn, img_as_ubyte(textonmap_rgb))
    FileLink(textonmap_img_fn)
...
FileLink(get_img_filename('dirmap'))
...
sptext_img_fn = save_img(sptext, 'sptext')
FileLink(sptext_img_fn)
...
save_img(saliency_map, 'saliency_map')
