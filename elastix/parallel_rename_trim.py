...
#                 [(stack, input_dir, output_dir, tmp_dir+'/'+stack+'_tmp%d'%i, 'thumbnail') for i in range(n_host)],
...
run_distributed3('%(script_dir)s/rename.py'%d,
                [(stack, input_dir, output_dir, tmp_dir+'/'+stack+'_tmp%d'%i, 'lossless') for i in range(n_host)],
                stdout=open('/tmp/log', 'ab+'))
print 'done in', time.time() - t, 'seconds'
