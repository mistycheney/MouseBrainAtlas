...
#                 [(stack, os.path.join(DATA_DIR, stack), os.path.join(DATAPROC_DIR, stack + '_lossless_renamed'),
#                 										os.path.join(DATAPROC_DIR, stack + '_lossless_renamed_jp2'), tmp_dir+'/'+stack+'_filename_map_%d'%i, 'lossless') 
...
#                 [(stack, os.path.join(DATAPROC_DIR, stack + '_lossless_renamed'), os.path.join(DATAPROC_DIR, stack + '_lossless_aligned_cropped'), f, l, 'lossless', x, y, w, h) 
#                     for f, l in first_last_tuples_distribute_over(first_sec, last_sec, n_hosts)],
#                 stdout=open('/tmp/log', 'ab+'))


t = time.time()
print 'generate downscaled version and grayscale version...',
