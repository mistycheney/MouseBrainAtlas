...
def combine_two_landmark_types(sec_ind):
    ...
    representative_cluster_scores_sorted, representative_clusters_sorted_by_score,     representative_cluster_indices_sorted_by_score, big_groups_sorted_by_score = zip(*representative_clusters)
    ...
    openBoundaries_scores_sorted, openBoundaries_edgeSets_sorted_by_score,     openBoundaries_indices_sorted_by_score = zip(*openBoundaries)
    ...
    overlapping_boundary_groups = group_clusters(all_supporters, dist_thresh = 0.1, metric=1)
    ...
    for edgeSet in nonoverlapping_edgeSets:
...
#         dm.save_pipeline_result(dm.visualize_edges(b[0], text=False, color=[0,255,0]), 'boundary%02dTemp'%i, 'jpg')
    
    vis = dm.visualize_edge_sets([b[0] for b in boundaries], text=False)
    dm.save_pipeline_result(vis, 'allBoundariesTemp', 'jpg')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Detect open boundary landmarks and combine both types')
