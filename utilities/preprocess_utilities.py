    ...
    #         sys.stderr.write('Step: %f seconds\n' % (time.time()-t)) # 0.6 second/step, roughly 200 steps takes 120s
        ...
        if discard:
            ...
            for l in np.unique(labeled_mask):
                if l != 0:
                    m = labeled_mask == l
                    s = np.count_nonzero(m)
                    if s > min_size:
                        final_masks.append(m)
                        sys.stderr.write('Final masks (size=%d) added.\n' % s)
...
# def save_submasks_one_section(stack, submask_decisions, submasks=None, submask_contour_vertices=None, fn=None, sec=None, img_shape=None):
#     """
#     Args:
#         submasks (dict):
