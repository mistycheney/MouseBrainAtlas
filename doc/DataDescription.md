...
- a (3,) int array representing the origin of this array with respect to a [reference frame](DomainDefinition.md), stored as `txt` file. Naming convention is `<volume_spec>_origin_wrt_<ref>.txt` where `<ref>` can be `wholebrain` or `canonicalAtlasSpace`.
