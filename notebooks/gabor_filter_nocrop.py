...
plt.imshow(dm.image, cmap=plt.cm.Greys_r)

# <codecell>

edge_masks = np.zeros_like(dm.image, dtype=np.bool)
edge_masks[max_kern_size/2:-max_kern_size/2, max_kern_size/2:-max_kern_size/2] = True
dm.mask = dm.mask & edge_masks

# <codecell>

plt.imshow(dm.mask, cmap=plt.cm.Greys_r)
...
cropped_segmentation = segmentation.copy()
cropped_segmentation[~dm.mask] = -1
...
plt.imshow(label2rgb(cropped_segmentation_relabeled))
