...
img_id = 244
img_name = 'PMD1305_%d.reduce2.region'%img_id
CACHE_DIR = '/home/yuncong/my_csd181_scratch/'
IMG_DIR = '/home/yuncong/my_csd181/ParthaData/PMD1305_reduce2/regions1/'
...
height, width, n_feat = features.shape
...
fig, axes = plt.subplots(ncols=4, nrows=features.shape[-1]/4, figsize=(20,10))
for i, ax in zip(range(features.shape[-1]), axes.flat):
    ax.matshow(features[...,i])
    ax.axis('off')
...
# segmentation = slic(img_rgb, compactness=3, sigma=20)
...
FileLink(img_name+'_superpixelized.jpg')

# <codecell>

img_superpixelized_patches = label2rgb(segmentation, img_rgb)

cv2.imwrite(img_name+'_img_superpixelized_patches.jpg', img_as_ubyte(img_superpixelized_patches))
FileLink(img_name+'_img_superpixelized_patches.jpg')
...
cv2.imwrite(img_name+'_textonmap_with_boundary_text.jpg', 
            img_as_ubyte(.5*textonmap_rgb + .5*img_superpixelized_text))
FileLink(img_name+'_textonmap_with_boundary_text.jpg')

# <codecell>

# FileLink(img_name+'_textonmap_with_boundary_text.jpg')

cv2.imwrite(img_name+'_superpixelized_text.jpg', 
            img_as_ubyte(img_superpixelized_text))
FileLink(img_name+'_superpixelized_text.jpg')
...
all_label = textonmap[gridy.ravel(), gridx.ravel()]
sp_label_hist = np.array([np.bincount(all_label[all_seg == s], minlength=num_textons) 
                 for s in range(n_superpixels)])

row_sums = sp_label_hist.sum(axis=1)
sp_label_hist_normalized = sp_label_hist.astype(np.float) / row_sums[:, np.newaxis]
...
# D = pdist(sp_label_hist_normalized, 'euclidean')
# D = pdist(sp_label_hist_normalized, chi2)
D = pdist(sp_label_hist_normalized, kl)
...
# 
# **First approach:** Use the distance between texton histogram of each superpixel vs. histogram of the overall image.

# <codecell>

intensitymap = sp_mean_intensity[segmentation].astype(np.int)

overall_intensity_hist, _ = np.histogram(intensitymap.flat, bins=range(257))
...
overall_intensity_hist_normalized_smoothed = gaussian_filter1d(overall_intensity_hist_normalized, 3)

axes[1].bar(np.arange(256), overall_intensity_hist_normalized_smoothed)
plt.show()

individual_saliency_score = overall_intensity_hist_normalized_smoothed[sp_mean_intensity.astype(np.int)]
saliency_score = np.exp(-individual_saliency_score)
...
overall_texton_hist = np.bincount(textonmap.flat)
overall_texton_hist_normalized = overall_texton_hist.astype(np.float) / overall_texton_hist.sum()

individual_saliency_score = np.array([kl(sp_hist, overall_texton_hist_normalized) for sp_hist in sp_label_hist_normalized])
...
for i, sp_hist in enumerate(sp_label_hist_normalized):
    ...
    saliency_score[i] += 2.*neighbor_term/(len(neighbors[i])-1)
...
plt.bar(np.arange(num_textons), sp_label_hist_normalized[95], alpha=.5, width=0.5, color='r', label='sp')
# plt.bar(np.arange(num_textons)+0.5, overall_texton_hist_normalized, alpha=.5, width=0.5, color='g', label='null')
plt.bar(np.arange(num_textons)+0.5, sp_label_hist_normalized[80], alpha=.5, width=0.5, color='g', label='null')
...
chi2(sp_label_hist_normalized[95], sp_label_hist_normalized[80])
...
#                             for j in range(n_superpixels) if j != i ])
#     saliency_score[i] = np.mean([np.exp(- sp_centroid_dist_matrix[i,j] / sp_diameter_mean) * 
#                                  hist_distance_matrix[i,j] 
#                             for j in range(n_superpixels) if j != i ])
...
for t in range(1, n_top_clusters+1):
    for i in saliency_score.argsort()[::-1]:
        if i not in chosen_superpixels:
            break
    ...
    #     print 'frontier', frontier
        ...
        for j in neighbors[i]:
#             if i==97:
#                 print j, hist_distance_matrix[curr_cluster,j].mean(), hist_distance_matrix[i,j],  hist_distance_matrix[j,i]
            if j != i and j not in curr_cluster and j not in chosen_superpixels\
            and hist_distance_matrix[curr_cluster,j].mean() < dist_thresh:
...
cv2.imwrite(img_name+'_seal.jpg', img_as_ubyte(selection_rgb))
FileLink(img_name+'_seal.jpg')
